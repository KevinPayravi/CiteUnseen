/*
DEPRECATED: This script is not being maintained and may or may not work.
 
This script scrapes MediaBiasFactCheck and pulls the most biased sources,
based on the binary left-right scale provided for each documented source.
 
Outputs into biased-sources.txt
*/
...
try {
    fs.unlinkSync('biased-sources.txt');
    console.log('Deleted biased-sources.txt')
  } catch(err) {
    console.error(err);
  }
                            ...
                            for (var j = 0; j < descriptionParagraphs.length; j++) {
                                ...
                                if (descriptionParagraphs[j].firstChild.data !== undefined) {
                                    if (descriptionParagraphs[j].firstChild.data.includes('Source:')) {
                                        ...
                                        fs.appendFile('biased-sources.txt', url + '\n', (err) => {
